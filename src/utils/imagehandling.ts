...
export const fileToBase64 = (file: File): Promise<string> => {
  ...
  /**
   * Checks if a model has vision capabilities
   * This is a simplified check - would need to be updated as more models get vision support
   */
  export const modelSupportsVision = (modelName: string): boolean => {
    // First check if this is a Groq vision model
    if (modelName.includes('llama-4-scout') || modelName.includes('llama-4-maverick')) {
      return true;
    }
    
    // Then check for Ollama vision models
    const ollamaVisionModels = [
      'llava', 'bakllava', 'llava-next', 'moondream', 
      'gemma', 'gemma3', 'gemma2', 'phi-3-vision'
    ];
    
    return ollamaVisionModels.some(model => 
